...
echo "=== Deploy ICE (treasury → factory cycles via CMC) ==="
cd "$PROJ_SS"
mkdir -p backend
...
echo "=== Sweep all ICE treasury ICP → factory cycles (amount 0 = all) ==="
...
echo -n "FACTORY ICP: "; dfx ledger balance --of-principal "$FACTORY" --network ic
dfx canister --network ic status "$FACTORY" 2>&1 | grep -E "Balance:|Status:" || true
dfx canister call ice getIceTreasuryIcpBalanceE8s --network ic
echo DONE
